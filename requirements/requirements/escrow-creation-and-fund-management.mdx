...
1. **WHEN** I create an escrow on Solana **THEN** the system SHALL lock SOL or SPL tokens in the Solana escrow program
2. **WHEN** I create an escrow on Ethereum **THEN** the system SHALL lock ETH or ERC-20 tokens in the Ethereum escrow contract via ZetaChain
3. **WHEN** cross-chain settlement occurs **THEN** the system SHALL use ZetaChain's Universal App to route messages and coordinate fund releases
4. **WHEN** I view my escrows **THEN** the system SHALL display escrows from all connected networks in a unified dashboard
5. **WHEN** network-specific errors occur **THEN** the system SHALL provide clear error messages indicating which blockchain encountered the issue

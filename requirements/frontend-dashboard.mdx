...
1. **WHEN** I visit the dashboard **THEN** the system SHALL display a "Connect Wallet" button prominently in the header
2. **WHEN** I click "Connect Wallet" **THEN** the system SHALL display a modal with wallet options (Phantom, Solflare, MetaMask, Tonkeeper)
3. **WHEN** I select a wallet **THEN** the system SHALL initiate the connection request and display a loading indicator
4. **WHEN** the connection succeeds **THEN** the system SHALL display my wallet address (truncated) and network indicator in the header
5. **WHEN** the connection fails **THEN** the system SHALL display a clear error message with troubleshooting steps

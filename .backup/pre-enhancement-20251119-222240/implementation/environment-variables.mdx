...
#### Primary AI Provider (Arcanum.ai)

#### `ARCANUM_API_KEY`
- **Description**: Arcanum.ai API key for task verification
- **Required**: Yes
- **Format**: API key string
- **Example**: `arcanum_live_1234567890abcdef1234567890abcdef`
- **Security**: Store securely, different keys for each environment
- **Acquisition**: Sign up at https://arcanum.ai and generate API key from dashboard

#### `ARCANUM_ENDPOINT`
- **Description**: Arcanum.ai API endpoint URL
- **Required**: Yes
- **Format**: `https://api.arcanum.ai/v1/analyze`
- **Default**: `https://api.arcanum.ai/v1/analyze`
- **Example**: `https://api.arcanum.ai/v1/analyze`

#### AI Fallback Providers (Optional)

These providers are used as fallbacks if Arcanum.ai is unavailable. The fallback order is: Arcanum.ai → OpenAI → Claude → Gemini

#### `OPENAI_API_KEY`
- **Description**: OpenAI API key for fallback verification
- **Required**: No (recommended for production)
- **Format**: API key string starting with `sk-`
- **Example**: `sk-1234567890abcdef1234567890abcdef1234567890abcdef`
- **Security**: Store securely, rotate regularly

#### `ANTHROPIC_API_KEY`
- **Description**: Anthropic (Claude) API key for fallback verification
- **Required**: No (recommended for production)
- **Format**: API key string starting with `sk-ant-`
- **Example**: `sk-ant-1234567890abcdef1234567890abcdef`
- **Security**: Store securely, rotate regularly

#### `GEMINI_API_KEY`
- **Description**: Google Gemini API key for fallback verification
- **Required**: No (recommended for production)
- **Format**: API key string
- **Example**: `AIzaSy1234567890abcdef1234567890abcdef`
- **Security**: Store securely, rotate regularly

#### Legacy AWS Configuration (Alternative - Not Recommended)

These variables are maintained for backward compatibility but Arcanum.ai is the recommended primary provider.

#### `AWS_REGION`
- **Description**: AWS region for Bedrock (if using as alternative)
- **Required**: No
- **Values**: AWS region codes
- **Example**: `us-east-1`
- **Note**: Only needed if using AWS Bedrock as an alternative provider

#### `AWS_ACCESS_KEY_ID`
- **Description**: AWS access key ID (if using Bedrock as alternative)
- **Required**: No
...
- **Description**: AWS secret access key (if using Bedrock as alternative)
- **Required**: No
...
#### `BEDROCK_MODEL_ID`
- **Description**: AWS Bedrock model identifier (if using as alternative)
- **Required**: No
...
ARCANUM_API_KEY=arcanum_test_your_key_here
...
const frontendEnvSchema = z.object({
  ...
  NEXT_PUBLIC_ARCANUM_API_KEY: z.string().optional(), // Only if client-side verification needed
});
...
const backendEnvSchema = Joi.object({
  ...
  ARCANUM_API_KEY: Joi.string().required(),
  ARCANUM_ENDPOINT: Joi.string().uri().required(),

...
Initiate a dispute for AI verification results.
...
AetherLock uses a robust AI provider fallback chain to ensure high availability:

**Primary Provider**: Arcanum.ai (specialized task verification)
**Fallback Chain**: Arcanum.ai → OpenAI → Claude → Gemini

If the primary provider (Arcanum.ai) is unavailable or returns an error, the system automatically falls back to the next provider in the chain. The verification response includes information about which provider was used:
...
```json
{
  ...
  "status": "completed",
  "aiProvider": "openai",
  "fallbackChain": ["arcanum-ai", "openai"],
  "fallbackReason": "ARCANUM_TIMEOUT",
  "result": "approved",
  "confidence": 0.91
}
```
...
Webhook endpoint for zkMe KYC status updates (internal use).
...
AetherLock supports webhooks for real-time notifications of escrow events.
...
- **Solana Devnet**: `https://api-devnet.aetherlock.io/v1`
- **ZetaChain Testnet**: `https://api-testnet.aetherlock.io/v1`
- **Somnia Testnet**: `https://api-somnia-testnet.aetherlock.io/v1`

### Mock Data

Use the `/mock` endpoints for testing without blockchain interactions:

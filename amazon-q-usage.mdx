...
**Problem Faced**: Bedrock API was returning 400 errors with "Invalid request body" when sending large evidence descriptions.

**Solution with Q**: Q recommended implementing token counting and chunking for large inputs. Added a preprocessing step to truncate evidence to 3000 tokens before sending to Bedrock.

```typescript
// Q-assisted solution for token management
...
### Problem 2: AWS Bedrock Rate Limiting

**Issue**: Getting throttled by Bedrock API during high-volume testing (429 errors).
...
**Q Conversation**:
- **Me**: "How to handle Bedrock rate limits in production? Getting 429 errors."
- **Q**: "Implement exponential backoff with jitter. Also consider request batching and caching results for identical evidence hashes."

**Solution Applied**:
```typescript
// Q-assisted retry logic with exponential backoff
async function verifyWithBedrockRetry(
  evidenceHash: string, 
  maxRetries: number = 3
): Promise<VerificationResult> {
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      return await verifyWithBedrock(evidenceHash)
    } catch (error) {
...
**Result**: ✅ 99.7% success rate even under load

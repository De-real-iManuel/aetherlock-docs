...
| **AWS Bedrock AI** | Evidence analysis and verification | Confidence scoring, decision making | IPFS, Chainlink Oracle |
...
| **Chainlink Oracle** | AI result authentication | Ed25519 signature verification | AWS Bedrock, Smart Contracts |
  ...
  async verifyEvidence(evidenceHash: string): Promise<VerificationResult> {
    ...
    const response = await bedrock.send(new InvokeModelCommand({
      ...
      body: JSON.stringify({
        ...
        messages: [{
          role: 'user',
          content: `Analyze this task evidence: ${evidence}`
        }],
        max_tokens: 1000
      })
    }));
...
```json
{
  "ai_service": "AWS Bedrock (Claude 3 Sonnet)",
  "fallback_ai": "OpenAI GPT-4, Anthropic Claude",
  "oracle": "Chainlink Functions",
